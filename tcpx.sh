...
installbbr(){
	...
	if [[ "${release}" == "centos" ]]; then
		if [[ ${version} = "6" ]]; then
			...
			if [[ ${bit} = "x86_64" ]]; then
				wget -N -O kernel-headers-c7.rpm https://chinagz2018-my.sharepoint.com/:u:/g/personal/ylx_chinagz2018_onmicrosoft_com/EXKLKQg-v19AgtRFBjzbDDsBpwESsN5mMXCU2dsCK26jdw?download=1
				wget -N -O kernel-c7.rpm https://chinagz2018-my.sharepoint.com/:u:/g/personal/ylx_chinagz2018_onmicrosoft_com/EUM8XYhvfnBHi9ftbXRq-HcBw_IFOxNQ4zHxhELiJ5WM3g?download=1
...
installxanmod(){
	...
	if [[ "${release}" == "centos" ]]; then
		...
		if [[ ${bit} = "x86_64" ]]; then
			wget -N -O linux-headers-d10.deb https://chinagz2018-my.sharepoint.com/:u:/g/personal/ylx_chinagz2018_onmicrosoft_com/EbDCkG7Ee9tHv8vJHjkAFygBO7C8YsOkusVcyQxpVKmOBw?download=1
			wget -N -O linux-image-d10.deb https://chinagz2018-my.sharepoint.com/:u:/g/personal/ylx_chinagz2018_onmicrosoft_com/EaL92Swps7xBtZy9DkbY-ZsBaUnsKYPLDis-2Ap6maEv8A?download=1
				
			dpkg -i linux-image-d10.deb
			dpkg -i linux-headers-d10.deb
				
			kernel_version="5.8.2-xanmod1"
		else
			echo -e "${Error} 还在用32位内核，别再见了 !" && exit 1	
		fi		
	fi
...
————————————内核管理————————————
 ${Green_font_prefix}1.${Font_color_suffix} 安装 BBR原版内核 - 5.6.15/5.7.7/5.8.2
 ${Green_font_prefix}2.${Font_color_suffix} 安装 BBRplus版内核 - 4.14.129
 ${Green_font_prefix}3.${Font_color_suffix} 安装 Lotserver(锐速)内核 - 多种
 ${Green_font_prefix}4.${Font_color_suffix} 安装 xanmod版内核 - 5.5.1/5.7.7/5.8.2
