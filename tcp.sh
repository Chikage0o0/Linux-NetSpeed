...
installbbr(){
	...
	if [[ "${release}" == "centos" ]]; then
		if [[ ${version} = "6" ]]; then
			...
			if [[ ${bit} = "x86_64" ]]; then
				wget -N -O kernel-headers-c7.rpm https://chinagz2018-my.sharepoint.com/:u:/g/personal/ylx_chinagz2018_onmicrosoft_com/EbcWVbtSVg9Ek5jFg8yAQIkBfSLvrcut1exFhOYoJaSQnw?download=1
				wget -N -O kernel-c7.rpm https://chinagz2018-my.sharepoint.com/:u:/g/personal/ylx_chinagz2018_onmicrosoft_com/EdCmE9bLBINCnrYDnTpiFhMBfKl0KVu5-F0Xudiq04pVdg?download=1

				yum install -y kernel-c7.rpm
				yum install -y kernel-headers-c7.rpm
			
				kernel_version="5.8.12"
			else
				echo -e "${Error} 还在用32位内核，别再见了 !" && exit 1
			fi	
...
installxanmod(){
	...
	if [[ "${release}" == "centos" ]]; then
		if [[ ${version} = "7" ]]; then
			if [[ ${bit} = "x86_64" ]]; then
				wget -N -O kernel-c7.rpm https://chinagz2018-my.sharepoint.com/:u:/g/personal/ylx_chinagz2018_onmicrosoft_com/EYazuHFxSdtOmuessXh7bEQBimKsrYH7n9TmLjKW-N67-Q?download=1
				wget -N -O kernel-headers-c7.rpm https://chinagz2018-my.sharepoint.com/:u:/g/personal/ylx_chinagz2018_onmicrosoft_com/EVpuwv1Qh_pPuYuPLFQempIBPnESmuyvOANv9cdvMOvCbw?download=1
				
				yum install -y kernel-c7.rpm
				yum install -y kernel-headers-c7.rpm
			
				kernel_version="5.8.12_xanmod1"
			else
				echo -e "${Error} 还在用32位内核，别再见了 !" && exit 1
			fi
		elif [[ ${version} = "8" ]]; then
		...
		if [[ ${bit} = "x86_64" ]]; then
			wget -N -O linux-headers-d10.deb https://chinagz2018-my.sharepoint.com/:u:/g/personal/ylx_chinagz2018_onmicrosoft_com/EUqF4uJ3ZwdOjL7TLwV5SEkBwwp2ld-osd4wJC6Lpp1DwA?download=1
			wget -N -O linux-image-d10.deb https://chinagz2018-my.sharepoint.com/:u:/g/personal/ylx_chinagz2018_onmicrosoft_com/EVK8Kj3bfw5Ajg7wauTTSWoBCZabWWIRbwImsM61p-LsKw?download=1
				
			dpkg -i linux-image-d10.deb
			dpkg -i linux-headers-d10.deb
				
			kernel_version="5.8.12-xanmod"
		else
			echo -e "${Error} 还在用32位内核，别再见了 !" && exit 1	
		fi		
	fi
...
————————————内核管理————————————
 ${Green_font_prefix}1.${Font_color_suffix} 安装 BBR原版内核 - 5.6.15/5.7.7/5.8.12
 ${Green_font_prefix}2.${Font_color_suffix} 安装 BBRplus版内核 - 4.14.129
 ${Green_font_prefix}3.${Font_color_suffix} 安装 Lotserver(锐速)内核 - 多种
 ${Green_font_prefix}4.${Font_color_suffix} 安装 xanmod版内核 - 5.5.1/5.7.7/5.8.12
